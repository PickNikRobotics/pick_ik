...
TEST_CASE("pick_ik::make_pose_cost_fn") {
    SECTION("Goal is frame") {
        ...
        auto const cost_fn = pick_ik::make_pose_cost_fn(goal, 0, 0);
    ...
    SECTION("Goal is frame, with rotation scale") {
        ...
        auto const cost_fn = pick_ik::make_pose_cost_fn(goal, 0, 0.5);
        CHECK(cost_fn({goal}) == Catch::Approx(0.));
    }
    ...
    SECTION("Goal is second index") {
        ...
        auto const cost_fn = pick_ik::make_pose_cost_fn(goal, 1, 0);
        CHECK(cost_fn({pick_ik::Frame{}, goal}) == Catch::Approx(0.));
    }
    ...
    SECTION("Negative rotation same as zero rotation scale") {
        ...
        auto const cost_fn_zero = pick_ik::make_pose_cost_fn(goal, 0, 0.);
        auto const cost_fn_neg = pick_ik::make_pose_cost_fn(goal, 0, -0.5);
    ...
    SECTION("Translation in two axis, square of distance") {
        ...
        auto const const_fn = pick_ik::make_pose_cost_fn(goal, 0, 0.5);
    ...
    SECTION("Rotation in one axis") {
        ...
        auto const const_fn = pick_ik::make_pose_cost_fn(goal, 0, 1.0);
    ...
    SECTION("Rotation in one axis, scaled 0.5") {
        ...
        auto const const_fn = pick_ik::make_pose_cost_fn(goal, 0, rotation_scale);
