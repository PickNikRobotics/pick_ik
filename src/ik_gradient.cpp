...
auto step(GradientIk& self, Robot const& robot,
  ...
  for (size_t i = 0; i < count; ++i) {
    self.working[i] = self.local[i] - gradient[i];
  }
  double const p1 = fitness_fn(self.working);
  ...
  if (std::isfinite(joint_diff)) {
    ...
    for (size_t i = 0; i < count; ++i) {
      self.working[i] = clip(robot, self.local[i] - gradient[i] * joint_diff,
                             active_variable_indexes[i]);
    }
  }

  auto const local_fitness = fitness_fn(self.local);
  auto const working_fitness = fitness_fn(self.working);

  // has solution improved?
  if (working_fitness < local_fitness) {
...
auto ik_search(std::vector<double> const& initial_guess, Robot const& robot,
  ...
  auto ik = GradientIk{initial_guess, initial_guess, initial_guess,
