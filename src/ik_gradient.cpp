...
GradientIk GradientIk::from(std::vector<double> const& initial_guess,
...
auto step(GradientIk& self, Robot const& robot,
  ...
  auto sum = std::accumulate(
      self.gradient.cbegin(), self.gradient.cend(), 0.0001,
      [](auto sum, auto value) { return sum + std::fabs(value); });
  double const f = 1.0 / sum * jd;
  std::transform(self.gradient.cbegin(), self.gradient.cend(),
                 self.gradient.begin(), [&](auto value) { return value * f; });
  ...
  for (size_t i = 0; i < count; ++i) {
    self.working[i] = clip(robot, self.local[i] - self.gradient[i] * joint_diff,
