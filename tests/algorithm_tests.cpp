...
TEST_CASE("gd_ik::set_indexes") {
  ...
  SECTION("Not setting any values returns original input") {
    auto res =
        gd_ik::set_indexes(input, std::vector<int>{}, std::vector<size_t>{});
    CHECK(res == input);
  }

  SECTION("Setting one value sets only that value") {
    auto res =
        gd_ik::set_indexes(input, std::vector<int>{-1}, std::vector<size_t>{0});
  ...
  SECTION("Setting last two values set only them and leave first alone") {
    auto res = gd_ik::set_indexes(input, std::vector<int>{-1, -2},
  ...
  SECTION("Setting bad index value throws") {
    CHECK_THROWS(gd_ik::set_indexes(input, std::vector<int>{-1},
                                    std::vector<size_t>{4}));
  }

  SECTION("Not enough indexes throws") {
    CHECK_THROWS(
        gd_ik::set_indexes(input, std::vector<int>{1}, std::vector<size_t>{}));
  }
}
