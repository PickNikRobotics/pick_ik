...
TEST_CASE("make_frame_tests") {
    auto const epsilon = 0.00001;
    auto const zero_frame = Frame{
        tf2::Vector3(0, 0, 0),
        tf2::Quaternion(0, 0, 0, 1),
    };

    SECTION("One goal, one test") {
        auto const test_fns = make_frame_tests({zero_frame}, epsilon);
        CHECK(test_fns.size() == 1);
    }

    SECTION("Zero threshold") {
        auto const test_fns = make_frame_tests({zero_frame}, 0.);
...
TEST_CASE("make_pose_cost_fn") {
    ...
    auto const translate_y2_frame = Frame{
        tf2::Vector3(0, 2, 0),
        tf2::Quaternion(0, 0, 0, 1),
    };
    auto const translate_xy1_frame = Frame{
        tf2::Vector3(1, 1, 0),
        tf2::Quaternion(0, 0, 0, 1),
    };
    auto const translate_xyz1_frame = Frame{
        tf2::Vector3(1, 1, 1),
        tf2::Quaternion(0, 0, 0, 1),
    };
    auto const rotate_x1_frame = Frame{
        tf2::Vector3(0, 0, 0),
        tf2::Quaternion(1, 0, 0, 1),
    };
    auto const rotate_y2_frame = Frame{
        tf2::Vector3(0, 0, 0),
        tf2::Quaternion(0, 2, 0, 1),
    };

    SECTION("Goal is frame") {
        auto const cost_fn = make_pose_cost_fn(zero_frame, 0, 0);
        CHECK(cost_fn({zero_frame}) == Catch::Approx(0.));
    }

    SECTION("Goal is frame, with rotation scale") {
        auto const cost_fn = make_pose_cost_fn(zero_frame, 0, 0.5);
        CHECK(cost_fn({zero_frame}) == Catch::Approx(0.));
    }

    SECTION("Goal is second index") {
        auto const cost_fn = make_pose_cost_fn(translate_y2_frame, 1, 0);
    ...
    SECTION("Negative rotation same as zero rotation scale") {
        auto const cost_fn_zero = make_pose_cost_fn(zero_frame, 0, 0.);
        auto const cost_fn_neg = make_pose_cost_fn(zero_frame, 0, -0.5);
    ...
    SECTION("Translation in two axis, square of distance") {
        ...
        auto const const_fn = make_pose_cost_fn(zero_frame, 0, 0.5);
    ...
    SECTION("Rotation in one axis") {
        ...
        auto const const_fn = make_pose_cost_fn(zero_frame, 0, 1.0);
    ...
    SECTION("Rotation in one axis, scaled 0.5") {
        ...
        auto const const_fn = make_pose_cost_fn(zero_frame, 0, rotation_scale);
...
TEST_CASE("make_pose_cost_functions") {
    auto const goal =
        Frame{tf2::Vector3(0.3327501714229584, -0.025710120797157288, 0.5902695655822754),
              tf2::Quaternion(0.9239557003781338,
                              -0.38249949508300274,
    ...
    SECTION("Function is same as make_pose_cost_fn") {
        auto const cost_fn = make_pose_cost_fn(goal, 0, rotation_scale);
        auto const cost_fns = make_pose_cost_functions({goal}, rotation_scale);
    ...
    SECTION("Two goals, two functions") {
        auto const cost_fns = make_pose_cost_functions({goal, frame}, rotation_scale);
